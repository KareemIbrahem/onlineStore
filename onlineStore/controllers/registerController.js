...
const googleRegister = async (req, res) => {
  const googleEmail = await googleVerify(req.body.credential).catch((error) => {
...
async function createUser(res, user) {
  ...
  try {
    ...
    if ((await usersModel.countDocuments({ email: user.email })) > 0) {
      throw "this email is already registred";
    } else {
      user.role = "customer";
